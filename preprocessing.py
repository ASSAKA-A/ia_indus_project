    ...
    for col in cat_cols:
        if fit:
            ...
            if le is not None:
                ...
                # Transform after handling unseen categories
                try:
                    df[col] = le.transform(df[col])
                except ValueError:
                    # If we still have issues, assign a default value
                    df[col] = -1
            else:
                df[col] = df[col].astype(str).fillna('NA')
    return df, encoders
...
def preprocess_data(x_train: pd.DataFrame, y_train: pd.DataFrame, x_test: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, Dict[str, LabelEncoder]]:
    ...
    x_test, _ = encode_categorical(x_test, encoders=encoders, fit=False)
...
if __name__ == "__main__":
    ...
    x_train_processed, y_train, x_test_processed, encoders = preprocess_data(x_train, y_train, x_test)
